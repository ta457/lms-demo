<div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
  ...
  <div class="w-full">
    <form action="{{ $action }}">
      ...
      <div class="flex items-center gap-4 flex-wrap">
        ...
        <div id="filterDropdown" class="z-10 hidden w-40 p-3 bg-white rounded-lg shadow dark:bg-gray-700">
          <ul class="space-y-2 text-sm" aria-labelledby="filterDropdownButton">
            <select name="sort_by_time"
              ...
              <option value="oldest" @if (request('sort_by_time')=='oldest' ) @selected(true) @endif>
                Oldest
              </option>
              <option value="latest" @if (request('sort_by_time')=='latest' ) @selected(true) @endif>
                Latest
              </option>
            </select>
            {{ $slot }}
        ...
        <div>
          <select name="filter_members"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full px-2 py-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
            <option value="members" @if (request('filter_members') == 'members')
              @selected(true)
              @endif
              >
              Members
            </option>
            <option value="all" @if (request('filter_members') == 'all')
              @selected(true)
              @endif
              >
              All
        ...
        <div class="flex gap-4">
          <div class="w-full">
            <input type="text" id="search" name="search"
              ...
              placeholder="Search by name/username" value="{{ request('search') }}">
          </div>
          <button type="submit"
  ...
  <div
    ...
    <button type="button" onclick="submitMemberList()"
    ...
    {{-- <div class="flex items-center space-x-3 w-full md:w-auto">
      ...
      <div id="actionsDropdown"
        ...
        <div class="py-1">
          <button data-modal-target="deleteAllModal" data-modal-toggle="deleteAllModal" id="fakeDeleteAllBtn"
            type="button"
